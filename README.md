...
- DC형 비중은 연 총급여를 기준으로 회사 부담금을 `총급여 / 12`로 추정해 더한 운용 점검 몫입니다.
- 총급여는 ISA 서민형/일반형 판정과 연금계좌 세액공제율 판정에 함께 사용합니다.
- 세후 자산 그래프는 S&P500 등 특정 지수 전망이 아니라 사용자가 고른 `연 기대수익률`을 양쪽에 동일 적용한 적립식 미래가치 비교입니다.
- 일반 경로는 연간 투자 가능 금액과 DC 추정액을 매년 적립하고, 절세 조합은 추정 절세/방어 금액까지 매년 재투자한다고 가정합니다.
